...
Cache.prototype = {
    ...
    sendHeaders: function(res, status, headers){
        ...
        for(var i=0; i<headerNames.length; i++){
            updatedHeaders[headerNames[i].toLowerCase()] = headers[headerNames[i]];
        }



        //remote date header and let NODE send the current date
        delete updatedHeaders.date;
        updatedHeaders['access-control-allow-origin'] = '*';

        if(this.method && this.method.toUpperCase() === 'OPTIONS') {
            updatedHeaders['access-control-allow-methods'] = 'GET,POST,DELETE,PUT';
            updatedHeaders['access-control-allow-headers'] = '*';
            status = 200;
        }

        res.writeHeader(status, updatedHeaders);
    },

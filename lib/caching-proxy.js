...
var CachingProxy = function(options) {
    ...
    var SERVER = {
        ...
        // Custom PMN vars
        GIT_CONFIG: {CODE: '', CONTENT: '', CONFIG: '', DIRTY: ''},
        ...
        onRequestReceived: function (req, res) {
            ...
            if (!served || !cache.exists()) {
                SERVER.GIT_CONFIG.DIRTY = 'DIRTY';
                console.log("caching-proxy.requesting: " + options.fullUrl);
        ...
        handleSpecialCases: function (req, res) {
            ...
            if (req.url.match(/^\/ping/)) {
                ...
                return true;
            } else if(req.url.match(/^\/reset/)){
                this.serveReset(req, res);
                return true;
            } else if(req.url.match(/^\/branch/)){
                this.serveBranch(req, res);
                return true;
            } else if(req.url.match(/^\/qa_status/)){
                this.serveQAStatus(req, res);
                return true;
        ...
        serveReset : function(req, res){
            SERVER.GIT_CONFIG.DIRTY = '';

            res.writeHeader('200', {'content-type': 'application/json'});
            res.end('Reset');
        },

        serveBranch : function(req, res){
            var url_parts = url.parse(req.url, true);
            var query = url_parts.query;
            res.writeHeader('200', {'content-type': 'application/json'});
            if(query.type == 'code') {
                SERVER.GIT_CONFIG.CODE = query.branch;
                res.end('Set Code = ' + query.branch);
            }
            else if(query.type == 'config') {
                SERVER.GIT_CONFIG.CONFIG = query.branch;
                res.end('Set Config = ' + query.branch);
            }
            else if(query.type == 'content') {
                SERVER.GIT_CONFIG.CONTENT = query.branch;
                res.end('Set Content = ' + query.branch);
            }
        },

        serveQAStatus : function(req, res){
            res.writeHeader('200', {'content-type': 'application/javascript'});
            var status = `document.addEventListener('DOMContentLoaded', function(){ 
                var e = document.createElement('div');
                e.innerHTML = '<table style="background: red;position: fixed;z-index: 100001;margin-top: 2px; padding: 5px; font-size: 0.5em;"><tr><td>Code:</td><td>${SERVER.GIT_CONFIG.CODE}</td></tr><tr><td>Config:</td><td>${SERVER.GIT_CONFIG.CONFIG}</td></tr><tr><td>Content:</td><td>${SERVER.GIT_CONFIG.CONTENT}</td></tr><tr><td>State:</td><td>${SERVER.GIT_CONFIG.DIRTY?"DIRTY":"CLEAN"}</td></tr></table>';
